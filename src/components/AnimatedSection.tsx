...
const AnimatedSection = ({ 
  ...
  const isInView = useInView(ref, { once: false, amount, margin: '-50px' });
...
export const LineReveal = ({ className = '', direction = 'left' }: LineRevealProps) => {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: false, amount: 0.5 });
  ...
  return (
    <motion.div
      ref={ref}
      className={`h-0.5 bg-primary ${className}`}
      initial={{ scaleX: 0 }}
